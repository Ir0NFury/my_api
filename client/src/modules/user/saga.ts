...
function* loginUser(action: PayloadAction<{email: string, password: string}>) {
    try {
        const response: LoginServiceResponse = yield call(AuthService.login, action.payload)
        localStorage.setItem('token', response.data.accessToken)
        yield put(userReducer.actions.userLoginSuccess(response.data.user))
    } catch(err: unknown) {
        yield put(userReducer.actions.userLoginError(err instanceof Error ? err.message : 'Unknown error'))
    }

}

function* registerUser(action: PayloadAction<{email: string, password: string}>) {
    try {
        const response: RegisterServiceResponse = yield call(AuthService.registration, action.payload)
        localStorage.setItem('token', response.data.accessToken)
        yield put(userReducer.actions.userRegisterSuccess(response.data))
    } catch(err: unknown) {
        yield put(userReducer.actions.userRegisterError(err instanceof Error ? err.message : 'Unknown error'))
    }
...
function* checkAuth() {
    try{
        const response: LoginServiceResponse = yield axios.get<AuthResponse>(`${API_URL}/refresh`, {withCredentials: true})
        localStorage.setItem('token', response.data.accessToken)
        yield put(userReducer.actions.userCheckAuthSuccess(response.data))
    } catch (err: unknown) {
        yield put(userReducer.actions.userCheckAuthError(err instanceof Error ? err.message : 'Unknown error'))
    }
}
